...
// input and storage files
const TEAMS_FILE:&str = "teams.txt";
const CHAMPIONSHIP_FILE:&str = "championship.txt";
...
impl fmt::Display for Stats {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(
            f,
            "{} | Points : {}, Wins : {}, Ties : {}, Losses : {} , Scored : {}, Conceded : {}, Diff : {}",
            self.name,
            self.get_points(),
            self.win,
            self.tie,
...
impl Ord for Stats {
    // reverse comparison wow (first in championship is first to be displayed)
...
impl PartialEq for Stats {
    fn eq(&self, other: &Stats) -> bool {
        self.get_points() == other.get_points() && self.scored == other.scored &&
...
impl Championship {
    fn set_result(&mut self, target_id: i32, receiving_team_count: i32, coming_team_count: i32) {
    ...
    fn show_rankings(&self) {
        ...
        for m in &self.matches {
            let mut receiving_team_stat = Stats {
                name: m.receiving_team.to_string(),
                scored: 0,
                conceded: 0,
                win: 0,
                tie: 0,
            ...
            let mut coming_team_stat = Stats {
                name: m.coming_team.to_string(),
                scored: 0,
                conceded: 0,
                win: 0,
                tie: 0,
...
fn create_championship() {
    ...
    let result = match result {
        Ok(r) => r,
        Err(e) => panic!("Error while loading teams file: {:?}", e),
    };
    ...
    let championship = Championship { teams, matches };
...
fn get_championship() -> Championship {
    ...
    serde_json::from_str(&championship_json).expect("Championship serialization failed! ")
}

// **************************Commands methods****************************************
fn update_championship(mid: i32, st1: i32, st2: i32) {
    let mut championship = get_championship();
    championship.set_result(mid, st1, st2);
    serialize_and_save_championship(&championship).expect("Championship save failed");
}

fn show_teams() {
    let championship = get_championship();
...
fn serialize_and_save_championship(championship: &Championship) -> Result<(), io::Error> {    
    let csstring =
        serde_json::to_string(championship).expect("Championship serialization failed! ");
    ...
    let mut f = OpenOptions::new()
        .write(true)
        .open("championship.txt")
        .expect("Unable to open file");

    f.write_all(csstring.as_bytes()).expect(
...
fn deserialize_championship() -> Result<String, io::Error> {
    let mut f = OpenOptions::new().read(true).open(CHAMPIONSHIP_FILE)?;
    let mut s = String::new();
...
fn main() {
    let app = App::new("Champ")
        ...
        .subcommand(SubCommand::with_name("create").about(
            "creates championship from teams enumerated in TEAMS_FILE file",
        ))
        .subcommand(SubCommand::with_name("teams").about("shows teams list"))
        .subcommand(SubCommand::with_name("calendar").about(
            "shows championship calendar",
        ))
        .subcommand(SubCommand::with_name("rankings").about(
            "show rankings of the championship",
        ))
        .subcommand(
                        ...
                        .validator(|v| {
                            v.parse::<i32>()
                                ...
                                .and_then(|v| if v > 0 {
                                    ...
                                    Err(String::from("match id cannot be negative nor zero"))
                                })
                                .map(|_| ())
                        }),
                )
                        ...
                        .validator(|v| {
                            v.parse::<i32>()
                                ...
                                .and_then(|v| if v >= 0 {
                                    ...
                                    Err(String::from("score cannot be negative"))
                                })
                                .map(|_| ())
                        }),
                )
    ...
    if app.subcommand_matches("create").is_some() {
        ...
        show_rankings();
    } else if let Some(cmd) = app.subcommand_matches("update") {
		
        update_championship(cmd.value_of("m").unwrap().parse().unwrap(),
							cmd.value_of("r").unwrap().parse().unwrap(),
							cmd.value_of("c").unwrap().parse().unwrap());
    } else {
